...
A tibble with a `wk` WKB column. The geometry is always `geom` and the feature
id always `fid`, whatever the driver called them (a shapefile says
`wkb_geometry` and `OGC_FID`); printing the source says when they were
renamed, and `write_to(geometry_name = , fid_name = )` chooses them on the way
